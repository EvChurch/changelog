...
export async function GET(req: Request) {
  ...
  if (teamId != null || since != null || before != null) {
    const where: { status: "accepted"; teamId?: string; acceptedAt?: { gte?: Date; lte?: Date } } = {
      status: "accepted",
    };
  ...
  return NextResponse.json({ error: "Missing role or teamId/since" }, { status: 400 });
}
...
export async function POST(req: Request) {
  ...
  const status = asDriver && isDriver
    ? ("pending_leader_review" as const)
    : ("pending_driver_review" as const);
  const source = asDriver && isDriver ? ("driver" as const) : ("member" as const);
